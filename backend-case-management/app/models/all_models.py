...
class Case(Base):
    ...
    assigned_officer = Column(Integer, ForeignKey("users.id"))
...
class InspectionRequest(Base):
    ...
    requested_officer = Column(Integer, ForeignKey("users.id"))
...
class Inspection(Base):
    ...
    latitude = Column(Float)
    longitude = Column(Float)
    physical_progress_observed = Column(Numeric(5,2))
    site_condition = Column(Text)
    financial_observation = Column(Text)
    general_observation = Column(Text)
...
class Evidence(Base):
    ...
    latitude = Column(Float)
    longitude = Column(Float)
    metadata_json = Column(Text) # "metadata" is reserved word sometimes, using metadata_json
